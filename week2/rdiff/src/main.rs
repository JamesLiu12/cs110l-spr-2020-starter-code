...
fn lcs(seq1: &Vec<String>, seq2: &Vec<String>) -> Grid {
    ...
    let mut table = Grid::new(seq1.len() + 1, seq2.len() + 1);
    for i in 0..seq1.len() {
        for j in 0..seq2.len() {
            if seq1[i] == seq2[j] {
                table.set(i + 1, j + 1, table.get(i, j).unwrap() + 1).unwrap();
            } else {
                table.set(i + 1, j + 1, cmp::max(table.get(i + 1, j).unwrap(), table.get(i, j + 1).unwrap())).unwrap();
            }
        }
    }
    table
...
fn main() {
    ...
    let file1_lines = read_file_lines(filename1).expect("Error when reading file 1");
    let file2_lines = read_file_lines(filename2).expect("Error when reading file 2");
