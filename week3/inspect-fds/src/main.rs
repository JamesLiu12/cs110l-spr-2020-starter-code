...
fn main() {
    ...
    let process = match ps_utils::get_target(target)
    .expect("problem calling ps or pgrep") {
    ...
    let children = ps_utils::get_child_processes(process.pid)
        .expect("problem getting child processes");
